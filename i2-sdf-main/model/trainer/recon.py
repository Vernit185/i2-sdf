...
class ReconstructionTrainer(pl.LightningModule):
    def __init__(self, conf, prog_bar: RichProgressBar, exp_dir, model_only=False, val_mesh=False, is_val=False, **kwargs) -> None:
        ...
        self.v_up = None
        self.h_floor = None
        self.pc_ground_cache = None
        if hasattr(self.train_dataset, 'pointcloud') and isinstance(self.train_dataset.pointcloud, torch.Tensor) and self.train_dataset.pointcloud.numel() > 0:
            n_cache = min(50000, self.train_dataset.pointcloud.size(0))
            idx_cache = torch.randperm(self.train_dataset.pointcloud.size(0))[:n_cache]
            self.pc_ground_cache = self.train_dataset.pointcloud[idx_cache, :3].clone()
    ...
    def training_step(self, batch, batch_idx):
        ...
        if getattr(self.loss, 'ground_weight', 0.0) > 0:
            ...
            if self.pc_ground_cache is not None and len(self.pc_ground_cache) >= B:
                rand_idx = torch.randint(0, len(self.pc_ground_cache), (B,), device=device)
                cand_pts = self.pc_ground_cache[rand_idx.cpu(), :3].to(device)
            elif hasattr(self.train_dataset, 'pointcloud') and isinstance(self.train_dataset.pointcloud, torch.Tensor) and len(self.train_dataset.pointcloud) >= B:
                rand_idx = torch.randint(0, len(self.train_dataset.pointcloud), (B,), device=device)
                cand_pts = self.train_dataset.pointcloud[rand_idx.cpu(), :3].to(device)
            else:
        ...
        current_step = getattr(self, '_custom_step', None)
        if current_step is None:
            current_step = self.global_step if getattr(self, '_trainer', None) is not None else 0
        loss_output = self.loss(model_outputs, ground_truth, current_step)
        if self.bubble_activated and self.loss.max_bubble_iter is not None and current_step >= self.loss.max_bubble_iter:
            # End bubble step
        ...
        self.last_loss_output = loss_output
