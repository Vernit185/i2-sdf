...
class I2SDFNetwork(nn.Module):
    ...
    def forward(self, input, predict_only=False):
        ...
        if self.training:
            ...
            if 'ground_points' in input:
                ground_points = input['ground_points']
                ground_sdf = self.implicit_network.get_sdf_vals(ground_points)
                output['ground_sdf'] = ground_sdf
                if 'num_ground_candidates' in input and 'num_ground_samples_per_col' in input:
                    output['num_ground_candidates'] = input['num_ground_candidates']
                    output['num_ground_samples_per_col'] = input['num_ground_samples_per_col']
                     ...
                     self.angular_weight * angular_loss + \
                      self.bubble_weight * bubble_loss + \
                       self.light_mask_weight * light_mask_loss + \
                        self.ground_weight * ground_loss
